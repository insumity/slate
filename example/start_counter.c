...
int add_events(int event_set) {
  ...
  int retval = PAPI_add_named_event(event_set, "PAPI_L3_TCA");
  if (retval != PAPI_OK) {
    printf("Couldn't add L3_TCA\n");
    exit(1);
    }
    
  retval = PAPI_add_named_event(event_set, "PAPI_L3_TCM");
  if (retval != PAPI_OK) {
  ...
  retval = PAPI_add_named_event(event_set, "MEM_LOAD_UOPS_RETIRED:L2_HIT");
  if (retval != PAPI_OK) {
    perror("couldn't add retired l2 hit\n");
    exit(1);
  }
...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    ...
    int wait_time_in_ms = 50 * one_ms_in_us;
