...
#    files=( "ONE_KYOTOCABINET_6" "ONE_KYOTOCABINET_10" "ONE_KYOTOCABINET_20" )
	...
	do
	    ...
	    #	    sudo ./schedule $APP_FILE_NAME foo. GREEDY true $POLICY 2>gathered_counter_results/$APP_FILE_NAME.pol_$POLICY.$ITERATION.results
	    sudo ./schedule $APP_FILE_NAME foo. GREEDY true $POLICY 2>/tmp/$APP_FILE_NAME.pol_$POLICY.$ITERATION.results
	...
	echo -e "$APP_FILE_NAME (policy: $POLICY) : (avg execution time: $result) (standard deviation: $stdy)" >> execution_times
