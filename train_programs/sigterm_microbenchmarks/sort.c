...
void sig_handler(int signo)
{
  ...
  char filename[500];
  sprintf(filename, "sort_total_loops_threads_%d_policy_%d_pin_%d", number_of_threads, pol, pin);
...
int main(int argc, char* argv[])
{
  ...
  int cnt = 0;
  for (int i = 0; i < number_of_threads; ++i) {
    sort_dt* pa = malloc(sizeof(sort_dt));
    pa->index = i;
    pa->array = malloc(sizeof(int) * ARRAY_SIZE);
    pa->size = ARRAY_SIZE;
    bzero(pa->array, pa->size);
    if (pthread_create(&threads[i], NULL, sort, pa)) {
