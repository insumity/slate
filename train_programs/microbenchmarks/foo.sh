...
for run in {1..48}
do
    rr_result=$(tail -n 5 rr_pinned_with_rr_threads$run | awk '{ sum += $3 } END { if (NR > 0) print sum / NR }')
    rr_result_n=$(printf '%.0f' $rr_result)
    echo "pame gia locality tora ..." 
    loc_result=$(tail -n 5 rr_pinned_with_loc_threads$run | awk '{ sum += $3 } END { if (NR > 0) print sum / NR }')
