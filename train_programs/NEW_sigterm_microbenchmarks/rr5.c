/* Benchmark that is better with RR although it has significant inter-socket activity */
#define _GNU_SOURCE
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
...
#define RANDOM_NUMBERS 6000 * 1000
...
void *inc(void *v)
{
  ...
  if (elapsed < 1) {
    sleep(1 - elapsed); // make sure the whole operation of alloc + bzero takes exactly 20s
  }
  ...
  for (long long int k = 0; k < repetions; ++k) {
    ...
    for (int l = 0; l < 4096 + 2048; ++l) {
      atomic_fetch_add(&counters[(l % 20) * ATOMIC_INTS_PER_CACHE_LINE], 1);
    }
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 0; i < number_of_threads; ++i) {
    ...
    dt->index = i;
    dt->random_numbers = malloc(sizeof(int) * RANDOM_NUMBERS);

    for (int i = 0; i < RANDOM_NUMBERS; ++i) {
      dt->random_numbers[i] = rand() % (16 * MB);
    }
