...
atomic_int shared_memory[16 * (8 * KB)];

void *inc(void *v)
{
  inc_dt* dt = (inc_dt *) v;

  long long size = 128 * MB;
  ...
  if (elapsed < 5) {
    sleep(5 - elapsed); // make sure the whole operation of alloc + bzero takes exactly 20s
  }
  ...
  for (long long int k = 0; k < repetions; ++k) {
    ...
    for (long long int times = offset; times < total - offset; times += offset) {
      for (long long int j = times; j < times + offset; ++j) {
	y[j] = y[random_numbers[j - 1]] + y[random_numbers[j - 5]];
      }
