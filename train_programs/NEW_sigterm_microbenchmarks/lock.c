...
long long int global_variable; // global variable protected by a lock
...
void *inc(void *v)
{
  ...
  while (true) {
    ...
    //int waiting_time_in_ms = (rand() % 50) * 1000;
    int waiting_time_in_ms = 5 * 1000;
...
void sig_handler(int signo)
{
  ...
  char filename[500];
  sprintf(filename, "lock_total_loops_threads_%d_policy_%d_pin_%d", number_of_threads, pol, pin);
