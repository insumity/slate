...
void print_perf_counters(bool is_rr, int result, long long values[], long long context_switches, double sockets_bw[], int number_of_threads) {
  ...
  long long cycles = values[6];
  long long intersocket = values[7] + values[8];
  long long intrasocket = values[4] - (values[0] + values[1]);

  
  // std::cerr << "= (intersocket: " << intersocket / ((double) uops_retired) << ") ";
  // std::cerr << "(intrasocket: " << intrasocket / ((double) uops_retired) << ") ";
  // std::cerr << "(uops/cycles: " << uops_retired / ((double) cycles) << ") ";
