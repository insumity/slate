...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    ...
    int read_fds[2][4] = {0}; // one for each integrated memory controller (IMCs 1, 3, 5, 7 always return 0)
    ...
    long long bw_per_socket[4] = {0};

    for (int imc = 0; imc < 4; ++imc) {
      for (int i = 0; i < 2; ++i) {
	long long results[3] = {0};
	read_perf_counter(read_fds[i][imc], results);
	bw_per_socket[imc] += results[0];

	read_perf_counter(write_fds[i][imc], results);
	bw_per_socket[imc] += results[0];
