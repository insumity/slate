...
void HGREEDY_reschedule(pid_t pid, int new_policy) {
  ...
  for (int i = 0; i < elems; ++i) {
    printf("%d: TID: %ld\n", i, *(pid_t *) values[i]);
    HGREEDY_release_hwc_extra(pid, *((pid_t *) values[i]));
    //HGREEDY_release_hwc(pid);
  }
  HGREEDY_release_hwc_extra(pid, pid);
...
void HGREEDY_release_hwc_extra(pid_t pid, pid_t tid) {
  sem_wait(state.lock);
  int hwc = *((int *) list_get_value(state.hwcs_per_pid, &tid, compare_pids));
