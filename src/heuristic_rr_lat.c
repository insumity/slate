...
void HRRLAT_init(pin_data** pin, mctop_t* topo) {
  ...
  state.lat_sockets = create_list();
  state.running_pids = create_list();
  state.lat_pids = create_list();
...
void* find_free_lat_socket(pid_t pid) {
  ...
  for (int i = 0; i < 4; ++i) {
    void* s = (void*) mctop_hwcid_get_socket(state.topo, i);

    if (list_get_value(state.lat_sockets, s, compare_voids) == NULL) {
      return s;
    }
    else {
      void* foo = list_get_value(state.lat_sockets, s, compare_voids);
...
void HRRLAT_new_process(pid_t pid, int policy) {
  ...
  *policy_pt = policy;
  ...
  if (policy == MCTOP_ALLOC_MIN_LAT_HWCS) {
    list_add(state.lat_pids, pid_pt, NULL);
  }
...
void HRRLAT_process_exit(pid_t pid) {
  ...
  int policy = *((int *) list_get_value(state.policy_per_pid, &pid, compare_pids));
  if (policy == MCTOP_ALLOC_MIN_LAT_HWCS) {
    list_remove(state.lat_pids, &pid, compare_pids);
  }
...
{
  list* l = state.running_pids;
  sem_wait(&(l->lock));
  struct node *tmp = l->head;
  while (tmp != NULL) {
    pid_t pid = *((pid_t*) tmp->key);

    int pol = *((int *) list_get_value(state.policy_per_pid, &pid, compare_pids));
    ...
    int num_tids;
    void** tids = list_get_all_values(state.tids_per_pid, &pid, compare_pids, &num_tids);

    for (int i = 0; i < num_tids; ++i) {
      pid_t tid = *((pid_t *) tids[i]);
      // get the hwc of the tid
      int* hwc_pt = (int *) list_get_value(state.hwcs_per_pid, &tid, compare_pids);
      if (hwc_pt == NULL) {
	...
	if (toSockets[j] == socket) {
      ...
      if (is_in) {
	int* pol_pt = (int *) list_get_value(state.policy_per_pid, &pid, compare_pids);
	if (pol_pt == NULL) {
	...
	while (cnt < state.total_hwcs) {
	  pin_data pd = state.pin[pol][cnt];
	  new_hwc = pd.core;
	  int node = pd.node;
	  
	  void* socket = (void*) mctop_hwcid_get_socket(state.topo, new_hwc);
	  bool is_in = false;
	  for (int j = 0; j < num; ++j) {
	    if (toSockets[j] == socket) {
	      is_in = true;
	  ...
	  if (!state.used_hwcs[new_hwc] && !is_in) {
	    goto end;
	  }
...
int get_unused_hwc_at_socket(int pol, void* socket) {
  ...
  while (cnt < state.total_hwcs) {
    ...
    void* s = (void*) mctop_hwcid_get_socket(state.topo, hwc);
    bool is_in = false;
    if (s == socket) {
      is_in = true;
    }

    if (!state.used_hwcs[hwc] && is_in) {
      return hwc;
...
int HRRLAT_get_hwc(pid_t pid, pid_t tid, int* ret_node) {
  ...
  if (policy == MCTOP_ALLOC_NONE) {
    ...
    *pid_pt = pid;
    ...
    int pol = policy;
    int num = list_elements(state.lat_pids);
    if (num <= 4) {
      ...
      if (list_get_value(state.lat_sockets, s, compare_voids) == NULL) {
	list_add(state.lat_sockets, s, pid_pt);
      }
    }
    else {
      // schedule wherever ...
...
void HRRLAT_release_hwc(pid_t pid) {
  ...
  list_remove(state.hwcs_per_pid, &pid, compare_pids);
  list_remove_all_data(state.lat_sockets, &pid, compare_pids);
